...
**Add attachment:** Use MCP `tool_attach_file(note_id, file_path)` for a single file, or `tool_attach_files(note_id, file_paths)` when the user hands over several files/a folder at once (list the folder first via `ls`/Glob) — one bad path in the batch never aborts the rest, check `failed` in the result and surface it. Ask for absolute file path(s) — must be under the user's home directory (path-traversal protection) and not a hidden/dotfile path (e.g. anything under a `.`-prefixed folder like `~/.ssh/` or `~/.config/` is refused, project-hub#134). Two distinct rejection reasons, don't conflate them when explaining a failure to the user: a path-traversal error means the file has to live under the home directory; a "hidden/dotfile" error means the specific path is blocked as a precaution even though it's under the home directory — ask for a non-hidden copy instead of implying the whole directory is off-limits.
